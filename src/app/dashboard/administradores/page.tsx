...
export default function AdminPage() {
  ...
  async function handleDelete(id: number) {
    ...
    try {
      await api.put(`/deladmin/${id}`);

      // Atualiza a lista sem recarregar a página
      setAdmins((prev) => prev.filter((item) => item.admId !== id));
    } catch (error) {
    ...
    <div className="flex flex-1 flex-col gap-6 p-6">
      ...
      <Card className="rounded-2xl shadow-sm">
        <CardHeader className="flex flex-row items-center justify-between">
          <CardTitle className="text-xl font-semibold">Administradores</CardTitle>

          <Link href="/dashboard/administradores/novo">
              ...
              <TableBody>
                {admins.length === 0 ? (
                  ...
                  admins.map((row) => (
                    ...
                    >
                      <TableCell className="w-20 font-medium">{row.admId}</TableCell>
                      ...
                      <TableCell className="text-right">
                        <Link href={`/dashboard/admin/edit/${row.admId}`}>
                      ...
                      <TableCell className="text-right">
                        <Button
                          variant="destructive"
                          size="icon"
                          onClick={() => handleDelete(row.admId)}
