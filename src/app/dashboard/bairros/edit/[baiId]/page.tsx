...
// ---------------- PAGE ----------------
export default function EditBairroPage() {
  const router = useRouter();
  const params = useParams();

  const baiId = params.baiId as string;
  ...
  useEffect(() => {
    async function loadBairro() {
      try {
        const response = await api.get(`/searchBairro/${baiId}`);
  ...
  // ---------------- UPDATE ----------------
  async function handleUpdateBairro(e: React.FormEvent) {
    e.preventDefault();

    try {
      setLoading(true);

      await api.put(`/altbairro/${baiId}`, {
        baiDescricao,
      });
      ...
      alert("Erro ao alterar o bairro");
    } finally {
      setLoading(false);
    }
  }
    ...
    <div className="w-full px-6 py-8 space-y-6">
      ...
      <Card className="w-full max-w-2xl mx-auto rounded-2xl shadow-sm">
        ...
        <CardContent className="space-y-6">
          <form onSubmit={handleUpdateBairro} className="space-y-5">
