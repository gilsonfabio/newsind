...
export default function EditConvenioPage() {
  ...
  const cnvId = params.cnvId as string;
  ...
  useEffect(() => {
    async function loadData() {
      try {
        const response = await api.get(`/searchConv/${cnvId}`);
        const data = response.data[0];

        setNomFantasia(data.cnvNomFantasia);
        setRazSocial(data.cnvRazSocial);
        setTelefone(data.cnvTelefone);
        ...
        setEstado(data.cnvEstado);
        setCep(data.cnvCep);
        setPassword(data.cnvPassword);
        setCanPassword(data.cnvCanPassword);
        setQtdParc(data.cnvQtdParc);
  ...
  async function handleUpdate(e: React.FormEvent) {
    ...
    try {
      ...
      await api.put(`/altconvenio/${cnvId}`, {
        cnvRazSocial,
        cnvNomFantasia,
        cnvCpfCnpj,
        cnvEmail,
      ...
      alert("Erro ao alterar convênio");
    } finally {
      setLoading(false);
    }
  }
